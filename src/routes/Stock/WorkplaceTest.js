...
export default class Workplace extends PureComponent {
  ...
  renderG6Graph = (data) => {
    ...
    kView.source(dv);
    kView.schema().position('time*range').color('trend', (val) => {
      if (val === '上涨') {
        return '#f04864';
      }
    ...
    }).shape('candle').tooltip('time*start*end*max*min', (time, start, end, max, min) => {
      return {
        name: time,
    ...
    barView.interval().position('time*volumn').color('trend', (val) => {
      if (val === '上涨') {
        return '#f04864';
      }
